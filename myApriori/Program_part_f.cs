...
namespace myApriori
{
    ...
    {
        #region Custom Types
        enum Metric { SupportCount, Support, Confidence, Lift }
        ...
        private Dictionary<kValue, Dictionary<ItemSet, Support>> _rules = new Dictionary<kValue, Dictionary<ItemSet, Support>>();
        #endregion
            ...
            if (splitItems.Length == 1)
            {
                switch (m)
                {
                    ...
                    case Metric.Lift:
                        ...
                        float supportWhole = getMetricValue(val.Replace('#', ','), Metric.SupportCount);
                        float supportAntecedent = getMetricValue(val.Split('#')[0], Metric.SupportCount);
                        float supportConsequent = getMetricValue(val.Split('#')[1], Metric.SupportCount);

                        float denominator = supportAntecedent * supportConsequent;
                        return denominator == 0 ? 0 : supportWhole / denominator;
        ...
        {
            _GenerateRules(Metric.Lift);
        }

        private void _GenerateRules(Metric m)
        {
            //create a dictionary of the same datatype as that of the frequent itemsets for rules but use # as delimiter
            //for each kitemset
                //generate rule combinations in the form {a}#{b} which means {a} -> {b}
                //find each combination's confidence
                    //add the rule and confidence into dictionary
                //add the dictionary to _rules
            Console.WriteLine("\nStarted Generating rules based on {0}...", m.ToString());
            Dictionary<string, float> ruleDict = new Dictionary<string, float>();
            for(int i = 1; i < _frequentItemSets.Count; i++)
            {
                GenerateRuleCombinations(i, m);
            }
            Console.WriteLine("\n...Finished generating rules");
        }

        private void GenerateRuleCombinations(int k, Metric m)
        {
            Dictionary<string, float> kRuleSet = new Dictionary<string, float>();
            foreach (KeyValuePair<string, float> kvp in _frequentItemSets[k])
            {
                for (int j = _frequentItemSets.Count - 1; j >= 0; j--)
                {
                    foreach (KeyValuePair<string, float> freqoneitemset in _frequentItemSets[j])
                    {
                        string rule = kvp.Key + "#" + freqoneitemset.Key;
                        if (!kRuleSet.ContainsKey(rule))
                            kRuleSet.Add(rule, getMetricValue(rule, m));
                    }
                    _rules[k] = kRuleSet;
                }
            }

            for(int l = 1; l < _rules.Count; l++)
            {
                Dictionary<string, float> validRule = new Dictionary<ItemSet, Support>();
                foreach(KeyValuePair<string,float> kvp in _rules[l])
                {
                    string antecedent = kvp.Key.Split('#')[0];
                    string consequent = kvp.Key.Split('#')[1];

                    if (!isIn(consequent, antecedent))
                    {
                        validRule.Add(kvp.Key, kvp.Value);
                    }
        ...
        private bool isIn(string consequent, string antecedent)
        {
            string[] allAntecedents = antecedent.Split(',');
            string[] allConsequents = consequent.Split(',');

            bool flag = false;
            foreach(string singleConsequent in allConsequents)
            {
                if (allAntecedents.Contains(singleConsequent))
                {
                    flag = true;
                    break;
                }
        ...
        public void DisplayFrequentkItemSets()
        {
            ...
            Console.WriteLine("\nNumber of Frequent Itemsets:{0}\nFinished displaying Frequent Itemsets", counter);
        }

        public void DisplayFrqClosedAndMaximalFrqItemSets()
        {
            //Generate_k_FrequentItemSets(k);
            Console.WriteLine("\nDisplaying Frequent Closed and Maximal Itemsets:\nItemSet \t\t Support Count\n");
            List<ItemSet> Displayed = new List<ItemSet>();
            int counterMaxF = 0;
            int counterClosedF = 0;
            for (int u = 0; u < _frequentItemSets.Count; u++)
            {
                foreach (KeyValuePair<ItemSet, Support> kvp in _frequentItemSets[u])
                {
                    if (kvp.Value >= _minsup)
                    {
                        //if (!Displayed.Contains(kvp.Key))
                        {
                            Dictionary<ItemSet, Support> supersets = getSuperSets(kvp.Key, u);
                            if (supersets != null)
                            {
                                Console.WriteLine(kvp.Key + " \t\t " + kvp.Value + " \t - Maximal Frequent");
                                counterClosedF++;
                                //Displayed.Add(kvp.Key);
                            }
                            else
                            {
                                ...
                                foreach(KeyValuePair<ItemSet, Support> superset in supersets)
                                {
                                    if (superset.Value > kvp.Value)
                                    {
                                        closedFlag = false;
                                        break;
                                    }
                                ...
                                if (closedFlag)
                                {
                                    Console.WriteLine(kvp.Key + " \t\t " + kvp.Value + " \t - Closed Frequent");
                                    counterMaxF++;
                                    //Displayed.Add(kvp.Key);
                                }
                            }
            ...
            Console.WriteLine("\nNumber of Closed Frequent Itemsets:{0}\n\nNumber of Maximal Frequent Itemsets:{1}\n\nFinished displaying Frequent Itemsets", counterClosedF, counterMaxF);
        }

        private Dictionary<ItemSet, Support> getSuperSets(ItemSet kVal, int k)
        {
            Dictionary<ItemSet, Support> retSuperSets = new Dictionary<string, float>();
            if (k + 1 < _frequentItemSets.Count)
            {
                foreach (KeyValuePair<ItemSet, Support> Superset in _frequentItemSets[k + 1])
                {
                    if (Superset.Key.Contains(kVal))
                    {
                        retSuperSets.Add(Superset.Key, Superset.Value);
                    }
                }
                return retSuperSets;
            }
        ...
        public void DisplayRules(float minconfOrLift)
        {
            Console.WriteLine("\nRule \t\t Confidence");
            for (int i = 1; i < _rules.Count; i++)
            {
                foreach(KeyValuePair<string, float> kvp in _rules[i])
                {
                    if (kvp.Value >= minconfOrLift)
                        Console.WriteLine(kvp.Key.Replace("#", "->") + " \t\t " + kvp.Value);
                }
            }
        }
    ...
    public class Program
    {
        ...
        private void WorkOnFile(string FilePath, char delimiter, float minsup, int kVal, float minconf)
        {
            ...
            if (isUserOK("Frequent Closed and Maximal Item Sets"))
                dh.DisplayFrqClosedAndMaximalFrqItemSets();

            dh.GenerateRules();
            if (isUserOK("Association rules"))
                dh.DisplayRules(minconf);
            dh = null;
        }
        ...
        public static void Main(string[] args)
        {
            ...
            catch (FormatException) { minsup = 0.05f; }
            catch (InvalidCastException) { minsup = 0.05f; }
            catch (InvalidOperationException) { minsup = 0.05f; }


            float minconf = 0.05f;
            Console.Write("Enter minconf value:");
            try { minconf = (float)Convert.ToDouble(Console.ReadLine()); }
            catch (FormatException) { minconf = 0.05f; }
            catch (InvalidCastException) { minconf = 0.05f; }
            catch (InvalidOperationException) { minconf = 0.05f; }

            Program pgrm = new Program();
            pgrm.WorkOnFile("zoo.data", ',', minsup, 7, minconf);// > 1000
