            ...
            if node.nodetype == "feature":
                ...
                if children is not None:
                    ...
                    if s_child is None:
                        ...
                        node.associated_dataholder.calc_entropy()
...
def get_gini(dataset, features_not_to_use=None):
    gini_of_s = 0.0
    sizeofds = len(dataset)
    if sizeofds > 0:
        n_of_feat = len(dataset[0])
        ...
        for uniq_class in uniqLst:   # last feature is the class
            ...
            p = float(numerator)/denominator
            if p > 0:
                gini_of_s += p * p # greater than one but less than log(len(unique_classes) to base 2)
    return gini_of_s
...
class dataHolder:
    ...
    def calc_gini(self):
        ...
        self.setMajorityClass()
        self.getPureClass()
        # GINI(S) = 1 - sigma(p*p) where p is the ratio of number of examples of particular class to total number of examples
        if self.len_dataset > 0:
            temp = 0.0
            denominator = len(self.dataset)
            for uniq_class in self.feature_uniqueValues[self.number_of_features - 1]:  # last feature is the class
                ...
                if p > 0:
                    temp += p * p
            # print "gini of dataset:\n" + str(dataset) + "\nis:" + str((1-temp))
            self.gini = 1 - temp

    def set_ginis(self, inp_feature):
        sum_of_ginis_weighted_by_proportion = 0
        s_dataset = len(self.dataset)
        for unique_feature_value in self.feature_uniqueValues[inp_feature]:
            temp_newds = self.getSubset(unique_feature_value, inp_feature)
            s_newds = len(temp_newds)
            ...
            if s_newds == 0:
                ...
                proportion = float(s_newds) / s_dataset
                gini_of_newds = get_gini(temp_newds)
            sum_of_ginis_weighted_by_proportion += proportion * gini_of_newds
        GINI = sum_of_ginis_weighted_by_proportion
        return GINI
    ...
    def calc_GINI(self):
        ...
        if self.len_dataset > 0:
            if self.gini_features_map is None:
                self.gini_features_map = dict()
            for feature_name in self.features_list:
                if feature_name not in self.featurestoexclude and feature_name!= self.number_of_features-1:
                    self.gini_features_map[feature_name] = self.set_ginis(feature_name)
    ...
    def calc_best_feature(self):
        if self.len_dataset > 0:
            if gain_or_gini == 0:
                ...
                self.best_feature = max(self.gini_features_map.iteritems(), key=operator.itemgetter(1))[0]
...
def buildtree(nd, dataholder, features_to_exclude=[], default_class=None):
    ...
    if len(dataholder.featurestoexclude) == len(dataholder.features_list) - 1:
        ...
        for uniq in dataholder.feature_uniqueValues[dataholder.best_feature]:
            new_dholder = dataHolder(dataholder.getSubset(uniq, dataholder.best_feature), None)
...
print "Enter 0 to use Info Gain or 1 for Gini:",
gain_or_gini = sys.stdin.readline()
try:
    gain_or_gini_flag = int(gain_or_gini)
...
print "starting 10 fold validation dataset construction..."
num_rows_original_data = len(autompg_data)
equalNum = num_rows_original_data / 10
DataSets = dict()

ten_fold_start_end = dict()
st = 0
en = 0
for i in range(0, 10):
    if en == 0:
    ...
    if i == 9:
        if en != len(autompg_data)-1:
            en = len(autompg_data)-1
    else:
        en = st + equalNum -1
    ten_fold_start_end[i] = [st, en]
    #print ten_fold_start_end[i]

def get_t_fold_split(start, end):
    retDS = list()
    for i in range(start, end):
        retDS.append(autompg_data[i])
    return retDS

def combineDatasets(not_to_be_used):
    retCombinedData = list()
    for k in range(0, 10):
        if k != not_to_be_used:
            for l in range(0, len(DataSets[k])):
                retCombinedData.append(DataSets[k][l])
    return retCombinedData

for di in range(0, 10):
    DataSets[di] = get_t_fold_split(ten_fold_start_end[di][0], ten_fold_start_end[di][1]+1)

for di in range(0, 10):
    Dataholder = dataHolder(combineDatasets(di), None)
...
'''
#Dataholder = dataHolder(autompg_data, None)
