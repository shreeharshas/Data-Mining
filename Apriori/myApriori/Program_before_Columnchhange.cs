...
namespace myApriori
{
    public class DataHandler
    {
        ...
        private void SetInitialCandidateAndFrequentSets()
        {
            ...
            _candidateItemSets.Add(0, oneItemCandidateSet); // update this also to handle the array index vs key properly like frequent itemset
            _frequentItemSets.Add(0, oneItemFrequentSet);
        }
        
        //private void Generate_k_CandidateItemsets(kValue k)
        private void GenerateNextCandidateItemsets(int k) //operates on kth frequent itemset _frequentItemSets[k]
        {
            foreach (KeyValuePair<ItemSet, Support> kvp in _frequentItemSets[k])
            {
                ...
                _candidateItemSets.Add(k + 1, dict);
            }
        }
        ...
        private void Generate_k_FrequentItemSets(kValue k)
        {
            ...
            if (k > 1) //start calculating for frequent 2 itemsets and further because frequent 1 itemset has already been generated at initialization
            {
                ...
                foreach (KeyValuePair<ItemSet, Support> kvp in GetFrequentItemSets(k - 1))
                {
                    ...
                    _frequentItemSets.Add(k, freqItemSets);
                }
            }
        ...
        public void ReadInput(string FilePath)
        {
            ...
            using (StreamReader sr = new StreamReader(FilePath))
            {
                ...
                DSFileContents.ForEach(DSRow => this._inputDataset.Rows.Add(DSRow.Split(',')));
        ...
        public void Apriori(kValue kval)
        {
            ...
            for (int k = 1; k < _k; k++)
            {
                ...
                GenerateNextCandidateItemsets(k - 1);//has to operate on kth frequent itemset _frequentItemSets[k]
                ...
                foreach (KeyValuePair<ItemSet, Support> kvp in _candidateItemSets[k])
                {
                    bool retain = true;
                ...
                {
                    if (_frequentItemSets[k + 1].ContainsKey(itm))
                    {
                        _frequentItemSets[k + 1].Remove(itm);
                    }
                }
            }
        }
    ...
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            catch (FormatException) { minsup = 0.04f; }
            catch (InvalidCastException) { minsup = 0.04f; }
            catch (InvalidOperationException) { minsup = 0.04f; }

            DataHandler dh = new DataHandler(0.2f);//input minsup - default is 0.5      //dh.ReadInput("Concrete.txt");//read to internal inputDataItemset
            dh.ReadInput("car.data");
            ...
            dh.Apriori(3); //Apriori for two itemsets
