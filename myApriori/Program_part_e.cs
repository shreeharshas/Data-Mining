...
namespace myApriori
{
    public class DataHandler
    {
        ...
        public void GenerateRules()
        {
            ...
            {
                GenerateRuleCombinations(i);
            }
        }

        private void GenerateRuleCombinations(int k)
        {
                ...
                for (int j = _frequentItemSets.Count - 1; j >= 0; j--)
                {
                    foreach (KeyValuePair<string, float> freqoneitemset in _frequentItemSets[j])
                    {
                        ...
                        if (!kRuleSet.ContainsKey(rule))
                            kRuleSet.Add(rule, getMetricValue(rule, Metric.Confidence));
                    }
                    _rules[k] = kRuleSet;
                }
            }
        ...
        public void DisplayRules(float minconf)
        {
            Console.WriteLine("\nRule \t\t Confidence");
            int counter = 0;
            for (int i = 1; i < _rules.Count; i++)
            {
                ...
                {
                    if (kvp.Value >= minconf && counter<10)
                    {
    ...
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            catch (FormatException) { minconf = 0.5f; }
            catch (InvalidCastException) { minconf = 0.5f; }
            catch (InvalidOperationException) { minconf = 0.5f; }

            Program pgrm = new Program();
            pgrm.WorkOnFile("car.data", ',', 0.05f, 7, minconf);// > 1000
            pgrm.WorkOnFile("nursery.data", ',', 0.05f, 7, minconf);// > 10000
            pgrm.WorkOnFile("kr-vs-kp.data", ',', 0.05f, 7, minconf);
